...
function groupBy<T>(items: T[], key: (t: T) => string): Record<string, GroupStats> {
  const map: Record<string, { w: number; l: number; p: number; pl: number; wagered: number }> = {};
  for (const item of items) {
    const k = key(item) || "unknown";
    if (!map[k]) map[k] = { w: 0, l: 0, p: 0, pl: 0, wagered: 0 };
    const bucket = map[k];
...
export async function GET() {
  ...
  const trades = rawTrades.map((row) => {
    const { game, ...rest } = row;
    return { ...rest, game, _league: game?.league, _lockType: rest.lock_type, _betType: rest.bet_type };
  });
